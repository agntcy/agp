...
use agp_datapath::{
    messages::{Agent, AgentType, encoder::DEFAULT_AGENT_ID, utils::AgpHeaderFlags},
    pubsub::{
        AgpHeader, SessionHeader,
        proto::pubsub::v1::{Message, SessionHeaderType},
    },
};
use tonic::async_trait;

use crate::session::{GwChannelSender, Id};
...
impl crate::timer::TimerObserver for RequestTimerObserver {
    async fn on_timeout(&self, timer_id: u32, timeouts: u32) {
        ...
        if self.send_gw.send(Ok(self.message.clone())).await.is_err() {
...
impl TopicHandler {
    ...
    pub async fn invite(&mut self, agent: &AgentType) {
        ...
        // TODO
        // return an error
        if self.send_gw.send(Ok(join.clone())).await.is_err() {
            error!("error sending invite message");
            return;
        }
    ...
    pub async fn ask_to_leave(&mut self, agent: &Agent) {
        ...
        // TODO
        // return an error
        if self.send_gw.send(Ok(leave.clone())).await.is_err() {
            error!("error sending invite message");
            return;
        }
    ...
    pub async fn join(&self) {
        ...
        let header = Some(AgpHeaderFlags::default().with_forward_to(self.conn));
        let sub = Message::new_subscribe(&self.source, &self.topic, None, header);

        // TODO
        // return an error
        if self.send_gw.send(Ok(sub)).await.is_err() {
            error!("error sending invite message");
        }
    }
    ...
    pub fn process_response_msg(&mut self, msg: Message) {
        ...
        match msg_type {
            SessionHeaderType::JoinReply => {
                ...
                let src = msg.get_agp_header().get_source();
                let key = src.clone().with_agent_id(DEFAULT_AGENT_ID);
                match self.pending_requests.get(&key) {
                    Some(t) => {
                        t.stop();
            ...
            SessionHeaderType::LeaveReply => {
                ...
                let src = msg.get_agp_header().get_source();
                match self.pending_requests.get(&src) {
                    Some(t) => {
                        t.stop();
                        self.pending_requests.remove(&src);
    ...
    fn create_request(
        ...
        let agp_header = Some(AgpHeader::new(&self.source, agent, agent_id, None));
...
mod tests {
    ...
    async fn test_invite_no_reply() {
        ...
        let source = Agent::from_strings("cisco", "default", "source", 0);
        let topic = AgentType::from_strings("cisco", "topic", "topic");

        let agent_to_invite = Agent::from_strings("cisco", "default", "to_invite", 5120);

        let mut h = TopicHandler::new(source, topic.clone(), SESSION_ID, 1, tx_gw);
    ...
    async fn test_invite_with_reply() {
        let (tx_gw, mut rx_gw) = tokio::sync::mpsc::channel(1);
        let source = Agent::from_strings("cisco", "default", "source", 12345);
        let topic = AgentType::from_strings("cisco", "topic", "topic");

        let agent_to_invite = Agent::from_strings("cisco", "default", "to_invite", 5120);

        let mut h = TopicHandler::new(source.clone(), topic, SESSION_ID, 1, tx_gw);
